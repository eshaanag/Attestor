...
| G' | Offline PDF report + cached AI remediation | Complete (dry-run remediation) | ReportLab PDF renders the genuine Cisco report offline: 2 pages, 9 failed controls, device identity, severity, mappings, and labelled dry-run remediation; remediation cache is opt-in and keyed by vendor/platform/rule |
